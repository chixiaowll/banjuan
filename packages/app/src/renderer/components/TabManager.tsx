...
export default function TabManager({ libraryPath, libraryName }: Props) {
  ...
  const refreshPluginViews = useCallback(() => {
    window.electronAPI.plugins.getViews().then(views => setPluginViews(views))
  }, [])
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      window.electronAPI.plugins.getViews().then(views => {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (!docId) return
      window.electronAPI.documents.get(docId).then((doc: any) => {
