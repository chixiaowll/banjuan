...
export default function NoteRenderService() {
  ...
  useEffect(() => {
    const unsub = window.electronAPI.noteRender.onRequest((noteId, requestId) => {
  ...
  const handleRendered = useCallback(async () => {
    ...
    try {
      const note = await window.electronAPI.notes.get(request.noteId)
      if (!note) {
        window.electronAPI.noteRender.sendResult(request.requestId, null)
        setRequest(null)
        return
      }
      ...
      if (note.type === 'handwriting') {
        ...
        window.electronAPI.noteRender.sendResult(request.requestId, dataUrl)
