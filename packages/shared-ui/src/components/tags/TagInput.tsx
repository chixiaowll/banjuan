...
export default function TagInput({ targetId, targetType, compact }: Props) {
  ...
  const loadTags = useCallback(async () => {
    const result = await api.tags.forTarget(targetId, targetType)
    setTags(result)
  }, [targetId, targetType])
  ...
  const handleAdd = async (tagName: string) => {
    ...
    await api.tags.assign(targetId, targetType, [trimmed])
    await loadTags()
  ...
  const handleRemove = async (tagName: string) => {
    await api.tags.unassign(targetId, targetType, tagName)
    await loadTags()
    document.dispatchEvent(new CustomEvent('tags-changed'))
  }
