...
export default function PluginViewHost({ pluginId, viewType }: Props) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        const source = await api.plugins!.getRendererSource(pluginId)
        if (cancelled) return
        ...
        const pluginApi = {
          ...
          rpc: (method: string, ...args: any[]) =>
            api.plugins!.rpc(pluginId, method, args),
          onMessage: (channel: string, handler: (data: any) => void) =>
            api.plugins!.onMessage(`plugin:${pluginId}:${channel}`, handler),
        ...
        if (typeof mod.activate === 'function') {
          const result = mod.activate(pluginApi)
